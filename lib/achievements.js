...
// ── Fetch achievements already unlocked by user ───────────────────────────────
...
async function unlockAchievement(userId, achievement, token) {
  ...
  // Ignore duplicate unlocks gracefully
...
// ── Master check — call after any action that could trigger achievements ──────
// actionType: 'task_complete' | 'goal_complete' | 'login' | 'points_updated'
// context: extra data relevant to the action (streak, totalTasks, etc.)
async function checkAchievements(userId, token, actionType, context = {}) {
  ...
  return newly; // array of { achievement, pointsAwarded }
}
...
async function shouldUnlock(achievement, userId, token, actionType, context) {
  ...
  switch (achievement.key) {
    ...
    case 'speed_10': {
      if (actionType !== 'task_complete') return false;
      // Count tasks completed within 1hr of creation
      const { data, error } = await db
        .from('tasks')
    ...
    // ── Early bird: completed before 8am ──
    case 'early_bird': {
      if (actionType !== 'task_complete') return false;
...
// Returns all achievements with unlocked status for a user (for the UI)
