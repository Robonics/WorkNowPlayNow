...
async function unlockAchievement(userId, achievement, token) {
  // Must use auth client so RLS sees the correct auth.uid()
  const db = getAuthClient(token);
  ...
  if (error) {
    // Unique constraint means already unlocked — skip silently
    if (error.code === '23505' || error.message?.includes('unique')) return null;
    console.error('unlockAchievement insert error:', error.message, error.code);
    throw error;
  }
...
async function checkAchievements(userId, token, actionType, context = {}) {
  console.log('checkAchievements called:', actionType, context);
  const [all, unlockedIds] = await Promise.all([
    getAllAchievements(),
    getUnlockedIds(userId, token),
  ]);
  console.log('Total achievements:', all.length, 'Already unlocked:', unlockedIds.size);
...
async function unlockAchievement(userId, achievement, token) {
  const db = getAuthClient(token);
  console.log('Attempting to unlock:', achievement.key, 'for user:', userId);
  ...
  console.log('Successfully unlocked:', achievement.key);
  // ... rest of function
}